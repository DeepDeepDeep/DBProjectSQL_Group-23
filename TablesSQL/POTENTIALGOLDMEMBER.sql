...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE NONEDITIONABLE VIEW "POTENTIALGOLDMEMBER" ("ID", "FIRST_NAME", "MID_NAME_INT", "LAST_NAME", "PHONE") AS 
...
OUTER LEFT JOIN PERSON ON BORROWER_ID = PERSON.ID;
   ...
   COMMENT ON TABLE "POTENTIALGOLDMEMBER"  IS 'Does an intersection to isolate silver members from borrow_details, then finds intersections regarding all months in the past year (from current date)'
;
